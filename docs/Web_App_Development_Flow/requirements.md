...
### 2.5. データ仕様変更・改善 (Phase 7)

- **CSV仕様の簡素化**
  - **ID列の廃止**: 読み込み時に行番号から自動的にIDを採番する仕様に変更（CSV作成の手間を軽減）。
  - **柔軟なフォーマット**: ヘッダー行さえあれば列順序は自由とし、解説列は省略可能とする。
  - **テンプレート配布**: スタート画面からフォーマット確認用のCSVテンプレートをダウンロード可能にする。
- **プロジェクト構成**
  - ドキュメント類をGit管理下に統合し、Python仮想環境 (`.venv`) を整備する。
